...
if (isset($_POST['register'])) {
    ...
    $stmt = $pdo->prepare("INSERT INTO administrateur (nom, prenom, email, mot_de_passe) VALUES (:nom, :prenom, :email, :mot_de_passe)");
    if ($stmt->execute([':nom' => $nom, ':prenom' => $prenom, ':email' => $email, ':mot_de_passe' => $mot_de_passe])){
        ...
        if ($stmt->rowCount() > 0) {
            ...
            // une mise à jour de valeur ou d'inserttion a été réalisé
        } else {
